...
We want to remove variants that we think are not good enough quality to retain in the analysis (e.g. --hwe and --geno filtering), OR are not useful for the analysis (e.g. --maf filtering). In this filtering, we
use a few different filters. 

`--hwe 0.001`: removes all variants where the p-value of the Hardy-Weinberg Equilibrium test is less than 0.001, indicating that this marker fails HWE. 
`--geno 0.02`: removes all variants where there is more than 2% missing data. 
`--maf 0.05`: removes all markers where the minor allele frequency is less than 5%.
</details>
...
<details>
    <summary>Answer: </summary>
The sample sizes should match up with what you have for the number from each population that you found using the fam file. The number of markers varies across populations, because the filtering steps detailed in the last answer are applied for each population separately (missingness and allele frequency in each population will be different).
</details>

3. Why is it important to separate each population before calculating LD?
<details>
    <summary>Answer: </summary>
    We separate then populations because we want a seperate N<sub>e</sub> estimate for each population. 
</details>
...
<details>
    <summary>Answer: </summary>
    We estimated the pairwise r<sup>2</sup> estimate of LD in this analysis. 
</details>
...
<details>
    <summary>Answer: </summary>
    To ensure that c=0.5 in our analysis formulas, we want to make sure that the locus pairs are on different chromosomes. So we want to exclude LD for locus pairs on the same chromosome. 
</details>
...
<details>
    <summary>Answer: </summary>
The effective population sizes are generally much smaller than the census pouplations sizes for all populations. 
</details>
...
<details>
    <summary>Answer: </summary>
    For each population, the number of segregating sites (S) is
    YRI: 3.410.000 
    HAN: 2.090.000
    JPT: 2.060.000
    CEU: 2.210.000
